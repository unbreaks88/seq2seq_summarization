...
def tokenize(sentences):
    ...
    for sentence in sentences:
        tokenized_sentence = list(regex.findall('[\p{Hangul}|\p{Han}|\p{Digit}\p{Latin}]+', sentence))
...
if __name__ == "__main__":
    ...
    docid, sumid, doc_dict, sum_dict = load_data("data/train.article.txt", "data/train.title.txt", "data/doc_dict.txt",
                                                 "data/sum_dict.txt", 30000, 30000)
    ...
    docid, sumid = load_valid_data("data/valid.article.filter.txt", "data/valid.title.filter.txt",
                                   doc_dict, sum_dict)
